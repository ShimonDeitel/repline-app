import XCTest
@testable import Repline

@MainActor
final class ReplineTests: XCTestCase {
    var store: Store!
    ...
    func testAddIncreasesCount() {
        ...
        store.add(WorkoutSet(exercise: "Test", weight: 1, reps: 1, rpe: 1))
    ...
    func testDeleteByItem() {
        let item = WorkoutSet(exercise: "Test", weight: 1, reps: 1, rpe: 1)
        store.add(item)
        store.delete(item)
